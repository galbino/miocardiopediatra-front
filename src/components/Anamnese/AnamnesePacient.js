    ...
    componentDidMount(){
        GetData("/user/"+this.state.user_id+"/anamnese").then(response => {
            if (response.errors.length === 0){
                if (response.data.length === 0) return this.setState({ statusSnack: true, displayMessage: "Usuário sem anamneses", variant: "warning", isLoading: false, no_anamneses: true })
    ...
    render(){
        ...
        const content = (
            <React.Fragment>
                ...
                />
                {this.state.no_anamneses ? 
                    <Typography variant="h5" gutterBottom align="center">Usuário sem Anamneses </Typography>
                :
